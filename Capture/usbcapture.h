...
public:
    SimpleUSBCapture() : m_running(false), m_frameCount(0), m_width(0), m_height(0), m_comInitialized(false), m_mfInitialized(false) {}

    ~SimpleUSBCapture() { Stop(); Shutdown(); }
    ...
    bool Initialize(int deviceIndex, ComPtr<ID3D11Device> d3d11Device) {
        ...
        if (deviceIndex < 0 || deviceIndex >= (int)count) {
            ...
            CoTaskMemFree(activateArray); return false;
        }
    ...
    void Stop() {
        ...
        { std::lock_guard<std::mutex> lock(m_mutex); while (!m_frameQueue.empty()) m_frameQueue.pop(); }
        { std::lock_guard<std::mutex> lock(m_poolMutex); while (!m_texturePool.empty()) m_texturePool.pop(); }
    }
...
private:
    ...
    ComPtr<ID3D11Texture2D> ConvertToTexture(IMFSample* sample) {
        ...
        {
            std::lock_guard<std::mutex> lock(m_poolMutex);
            if (!m_texturePool.empty()) { texture = m_texturePool.front(); m_texturePool.pop(); }
        }
        if (!texture) {

...
public:
    ...
    bool Initialize(const GPUInfo& gpuInfo, const RecordingSettings& settings,
        ...
        m_d3d11Device = d3d11Device; m_d3d11Context = d3d11Context;
        if (!m_d3d11Device || !m_d3d11Context) return false;
    ...
    bool EncodeFrame(const CapturedFrame& frame, std::vector<EncodedPacket>& outPackets) {
        ...
        if (m_useSystemMemory) {
            avFrame->format = AV_PIX_FMT_NV12; avFrame->width = m_codecContext->width; avFrame->height = m_codecContext->height;
            ...
            if (SUCCEEDED(m_d3d11Context->Map(m_stagingTexture.Get(), 0, D3D11_MAP_READ, 0, &mapped))) {
                const uint8_t* inData[1] = { (const uint8_t*)mapped.pData }; int inLinesize[1] = { (int)mapped.RowPitch };
            ...
            avFrame->format = AV_PIX_FMT_D3D11; avFrame->hw_frames_ctx = av_buffer_ref(m_hwFramesCtx);
            if (av_hwframe_get_buffer(m_hwFramesCtx, avFrame, 0) < 0) { av_frame_free(&avFrame); return false; }
            ID3D11Texture2D* dstTex = (ID3D11Texture2D*)avFrame->data[0]; UINT dstSub = (UINT)(intptr_t)avFrame->data[1];
...
private:
    ...
    bool TryInitHardware(const GPUInfo& gpuInfo, const RecordingSettings& settings) {
        ...
        if (!m_useSystemMemory) {
            ...
            framesCtx->width = settings.width; framesCtx->height = settings.height; framesCtx->initial_pool_size = 64;
            if (av_hwframe_ctx_init(m_hwFramesCtx) < 0) return false;
            m_codecContext->hw_frames_ctx = av_buffer_ref(m_hwFramesCtx);
            m_codecContext->pix_fmt = AV_PIX_FMT_D3D11; m_codecContext->sw_pix_fmt = AV_PIX_FMT_BGRA;
        } else {
        ...
        if (gpuInfo.encoderType == EncoderType::NVIDIA_NVENC) {
            av_opt_set(m_codecContext->priv_data, "preset", "p4", 0); av_opt_set(m_codecContext->priv_data, "tune", "hq", 0);
            av_opt_set(m_codecContext->priv_data, "rc", "constqp", 0); av_opt_set_int(m_codecContext->priv_data, "qp", 15, 0);
        } else if (gpuInfo.encoderType == EncoderType::AMD_AMF) {
            av_opt_set(m_codecContext->priv_data, "quality", "balanced", 0);
            av_opt_set(m_codecContext->priv_data, "rc", "cqp", 0); av_opt_set_int(m_codecContext->priv_data, "qp_i", 15, 0); av_opt_set_int(m_codecContext->priv_data, "qp_p", 15, 0);
        } else if (gpuInfo.encoderType == EncoderType::INTEL_QSV) {
            av_opt_set(m_codecContext->priv_data, "preset", "balanced", 0); av_opt_set_int(m_codecContext->priv_data, "global_quality", 15, 0); av_opt_set_int(m_codecContext->priv_data, "async_depth", 2, 0);
        }
        if (avcodec_open2(m_codecContext, codec, nullptr) < 0) return false;
        m_packet = av_packet_alloc(); return true;
    }
    ...
    bool TryInitSoftware(const RecordingSettings& settings) {
        ...
        m_codecContext = avcodec_alloc_context3(codec);
        if (!m_codecContext) return false;
        m_useSystemMemory = true; m_codecContext->pix_fmt = AV_PIX_FMT_NV12;
        if (!InitStagingTexture(settings) || !InitSwsContext(settings)) return false;
        ConfigureCommon(settings);
        av_opt_set(m_codecContext->priv_data, "preset", "ultrafast", 0); av_opt_set(m_codecContext->priv_data, "tune", "zerolatency", 0); av_opt_set_int(m_codecContext->priv_data, "crf", 15, 0);
        if (avcodec_open2(m_codecContext, codec, nullptr) < 0) return false;
        m_packet = av_packet_alloc(); spdlog::info("Using Software Encoder: {}", codec->name); return true;
    }

    void ConfigureCommon(const RecordingSettings& settings) {
        m_codecContext->width = settings.width; m_codecContext->height = settings.height;
        m_codecContext->time_base = { 1, (int)settings.fps }; m_codecContext->framerate = { (int)settings.fps, 1 };
        m_codecContext->gop_size = (int)settings.fps * 2; m_codecContext->max_b_frames = 0; m_codecContext->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;
    }
    ...
    ComPtr<ID3D11Device> m_d3d11Device; ComPtr<ID3D11DeviceContext> m_d3d11Context;
    AVCodecContext* m_codecContext; AVPacket* m_packet; AVBufferRef* m_hwDeviceCtx, * m_hwFramesCtx;
    bool m_useSystemMemory; SwsContext* m_swsContext; ComPtr<ID3D11Texture2D> m_stagingTexture;

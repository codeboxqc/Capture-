...
public:
    ...
    bool Initialize(const GPUInfo& gpuInfo, const RecordingSettings& settings,
        ...
        if (gpuInfo.encoderType == EncoderType::NVIDIA_NVENC) {
            ...
            // Intel UHD 630 and older don't support HEVC encoding well
            // Always use H264 QSV for best compatibility
            codecName = "h264_qsv";
            codec = avcodec_find_encoder_by_name("h264_qsv");
            if (codec) {
                spdlog::info("Intel QSV: Using H264 for best compatibility");
            }
        }
    ...
    retry_init:
        ...
        // Quality preset: 0=Lossless(0), 1=High(5), 2=Medium(15), 3=Low(25)
        int targetQuality = 0;
        switch (settings.qualityPreset) {
        case 0: targetQuality = 0; break;   // Lossless
        case 1: targetQuality = 5; break;   // High
        case 2: targetQuality = 15; break;  // Medium
        case 3: targetQuality = 25; break;  // Low
        default: targetQuality = 0; break;
        }
        spdlog::info("Quality preset: {} (QP: {})", settings.qualityPreset, targetQuality);

    open_codec:
        // FIX: Use actual GPU encoder type, not SOFTWARE just because we use system memory
        // System memory affects texture handling, not encoder options
        EncoderType effectiveType = gpuInfo.encoderType;
    ...
    bool EncodeFrame(const CapturedFrame& frame, std::vector<EncodedPacket>& outPackets) {
        ...
        while (true) {
            ...
            ep.pts = ep.pkt->pts;
            ep.keyframe = (ep.pkt->flags & AV_PKT_FLAG_KEY) != 0;
...
private:
    ...
    bool ConfigureEncoderOptions(EncoderType encoderType, int targetQuality) {
        ...
        if (encoderType == EncoderType::NVIDIA_NVENC) {
            ...
            // FIX: tune=lossless ONLY works with rc=lossless (QP=0)
            // For other quality levels, use tune=hq
            if (targetQuality == 0) {
                ret |= av_opt_set(m_codecContext->priv_data, "tune", "lossless", 0);
                ret |= av_opt_set(m_codecContext->priv_data, "rc", "lossless", 0);
                spdlog::info("NVENC: Lossless mode enabled");
            }
            else {
            ...
            if (targetQuality == 0) {
                ...
                if (av_opt_set(m_codecContext->priv_data, "profile", profile, 0) < 0) {
                    spdlog::warn("NVIDIA 4:4:4 profile '{}' not supported", profile);
                }
                else {
                    spdlog::info("NVIDIA 4:4:4 profile '{}' enabled for perfect text quality", profile);
                }
            }
            else {
                const char* profile = (m_codecContext->codec_id == AV_CODEC_ID_HEVC) ? "main" : "high";
                av_opt_set(m_codecContext->priv_data, "profile", profile, 0);
            }
            ...
            // Enable AQ for non-lossless (improves quality distribution)
            if (targetQuality > 0) {
            ...
            // AMD AMF doesn't support true lossless (QP=0)
            int amfQuality = targetQuality;
            if (amfQuality == 0) {
                amfQuality = 1;  // Use near-lossless for AMD
                spdlog::info("AMD AMF: Using near-lossless (QP=1) - true lossless not supported");
            }
            ret |= av_opt_set(m_codecContext->priv_data, "quality", "quality", 0);
            ret |= av_opt_set(m_codecContext->priv_data, "rc", "cqp", 0);
            ret |= av_opt_set_int(m_codecContext->priv_data, "qp_i", amfQuality, 0);
            ret |= av_opt_set_int(m_codecContext->priv_data, "qp_p", amfQuality, 0);
        }
        else if (encoderType == EncoderType::INTEL_QSV) {
            // Intel QSV doesn't support true lossless (QP=0)
            int qsvQuality = targetQuality;
            if (qsvQuality == 0) {
                qsvQuality = 1;  // Use near-lossless for Intel
                spdlog::info("Intel QSV: Using near-lossless (QP=1) - true lossless not supported");
            }
            // QSV uses global_quality for ICQ mode (1-51, lower = better)
            m_codecContext->global_quality = qsvQuality;
            m_codecContext->max_b_frames = 0;
            spdlog::info("Intel QSV configured: global_quality={}", qsvQuality);
        }
